...
public class ChoiceNode : Node 
{
	...
	public IEnumerator Running()
    {
        ...
        for (int x = 0; x < Number_Of_Choices; x++)
        {
            if (Button_Events[x].GetPersistentEventCount() > 0)
            {
                ...
                if (Show_Choice_Was_Selected_Before[x] && (Choice_Been_Clicked_Before[x] || StatsManager.Compare_Bool_Stat_To(Button_Text[x], true)))
    ...
    public void Choice_Clicked(int choice_number)
    {
        ...
        StatsManager.Set_Boolean_Stat(Button_Text[choice_number], true);
    }
